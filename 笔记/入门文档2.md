...
学习 Kafka 入门知识看这一篇就够了！（万字长文）(1)

---

Kafka 系列的阶段性总结（万字长文，做好准备）

![](assets/network-asset-m78gde2w93-20251027175225-p30exon.png)

这是Java 极客技术的第 265 篇原创文章
...
![](assets/network-asset-pyuuucobqb-20251027175225-krfbbec.png)
...
![](assets/network-asset-tg4jlig3il-20251027175225-epp7i8l.jpeg)
...
![](assets/network-asset-wtzrp9f7zk-20251027175225-tiow759.png)

点对点模式的消息队列

如果一个生产者或者多个生产者产生的消息能够被多个消费者同时消费的情况，这样的消息队列成为发布订阅模式的消息队列

![](assets/network-asset-shqz2wriho-20251027175225-l4nml6o.jpeg)

发布-订阅模式的消息队列

#### **Kafka 系统架构**

![](assets/network-asset-c5b92u7wmv-20251027175225-wmxepeq.jpeg)
...
Kafka 安装我在 Kafka 系列第一篇应该比较详细了，详情见**[带你涨姿势的认识一下kafka](/developer/tools/blog-entry?target=https%3A%2F%2Fmp.weixin.qq.com%2Fs%3F__biz%3DMzU2NDg0OTgyMA%3D%3D%26mid%3D2247484570%26idx%3D1%26sn%3D1ad1c96bc7d47b88e976cbd045baf7d7%26chksm%3Dfc45f969cb32707f882c52d7434b2c0bf2ccbbc2cd854e1dc5c203deb8ae9c1831cf216e8bad%26token%3D674527772%26lang%3Dzh_CN%26scene%3D21%23wechat_redirect&objectId=1547380&objectType=1&contentType=undefined)** 这篇文章。
...
默认情况下，kafka 会使用三种方式来**自动创建主题**，下面是三种情况：
...
![](assets/network-asset-mve2myskq3-20251027175225-phqb6h7.jpeg)
...
![](assets/network-asset-8raqt7ch9b-20251027175225-rjxmjqz.png)
...
![](assets/network-asset-hnjctcqpsb-20251027175226-ytuk8tj.png)

上图表示的就是**轮询策略**，轮询策略是 Kafka Producer 提供的默认策略，如果你不使用指定的轮训策略的话，Kafka 默认会使用顺序轮训策略的方式。

**随机轮询**

随机轮询简而言之就是随机的向 partition 中保存消息，如下图所示

![](assets/network-asset-14rg92cjq9-20251027175226-y9wz7wk.jpeg)
...
![](assets/network-asset-7ccps365gi-20251027175226-c6ixkk7.jpeg)

一些细节：不同key可能映射到同一分区；不同key之间的顺序并不确定
...
Kafka 的消息分为两层：消息集合 和 消息。一个消息集合中包含若干条日志项，而日志项才是真正封装消息的地方。Kafka 底层的消息日志由一系列消息集合日志项组成。Kafka 通常不会直接操作具体的一条条消息，它总是在**消息集合**这个层面上进行`写入`操作。
...
在上一篇文章 **带你涨姿势的认识一下kafka**中，我们主要介绍了一下 kafka 集群搭建的参数，本篇文章我们来介绍一下 Kafka 生产者重要的配置，生产者有很多可配置的参数，在文档里（http://kafka.apache.org/documentation/#producerconfigs）都有说明，我们介绍几个在内存使用、性能和可靠性方面对生产者影响比较大的参数进行说明
...
![](assets/network-asset-jpiviqdurd-20251027175226-s149483.jpeg)

![](assets/network-asset-d5gb4a8nnx-20251027175226-haw40ap.jpeg)
...
![](assets/network-asset-st7pe0v2jf-20251027175226-vsetfj7.jpeg)
...
![](assets/network-asset-n0tp9pu2rv-20251027175226-a4jfxuj.jpeg)

这样一来，消费者的消费能力就大大提高了，但是在某些环境下比如用户产生消息特别多的时候，生产者产生的消息仍旧让消费者吃不消，那就继续增加消费者。

![](assets/network-asset-tay2vu6g6i-20251027175226-qwv8bfm.jpeg)

如上图所示，每个分区所产生的消息能够被每个消费者群组中的消费者消费，如果向消费者群组中增加更多的消费者，那么多余的消费者将会闲置，如下图所示

![](assets/network-asset-ljxud6pebk-20251027175226-lpaq68a.jpeg)
...
![](assets/network-asset-nx35ss3mhh-20251027175226-godcd6c.jpeg)
...
![](assets/network-asset-aec6iuvjxf-20251027175226-10gho18.jpeg)
...
|语义|机制|优缺点|
| ------| -------------------------------------------------| --------------------------------|
|**At most once（至多一次）**|先提交 offset 再处理消息|不会重复，但可能丢消息|
|**At least once（至少一次）**|先处理消息再提交 offset|不丢消息，但可能重复|
|**Exactly once（恰好一次）**|Kafka Streams / 事务性 producer + consumer 协调|不丢也不重，但实现复杂，性能低|

![](assets/network-asset-o8hvhu1edq-20251027175226-cu81i8u.jpeg)
...
此篇文章是关于 kafka 的入门总结，是 cxuan Kafka 系列的第三篇文章，文章大部分是对之前三篇的汇总，也补充了一些新加的东西，如果觉得好，欢迎转发和在看。
